 ...
 * Калькулятор стоимости IT-услуг в Кыргызстане и обработка форм заявки
 * Локализирован для рынка Кыргызстана с поддержкой платежных систем KG
 */

// ==================== КОНФИГУРАЦИЯ ====================

/**
 * Конфигурация для Кыргызстана
...
const COMPANY_CONFIG = {
    ...
    address: 'Бишкек, пр. Чуй, д. 123, офис 45',
    country: 'Кыргызстан',
    currency: 'KGS (сом)',
    exchangeRate: 89, // 1 USD = 89 KGS в 2024 году
 ...
 * 2. Создайте форму на почте info@techpro.kg
 ...
 * - Средние ставки разработчиков в Бишкеке, КР
 * - Курс USD/KGS: 89 сом за доллар (актуальный на 2024)
 * - Конкурентный анализ с соседними странами (Россия, Казахстан)
 * - Налоговые льготы для IT-компаний КР (патентная система, 10% налог)
 * 
 * Поддерживаемые платежные системы Кыргызстана:
 * - Apelsin (ООО "МегаКом")
 * - Elfin (электронный кошелек)
 * - ELSOM (система расчетов)
 * - Банковские переводы через НБКР
 */
const baseCosts = {
 ...
 * Курс KGS/USD: 89 сом = 1 USD (актуальный курс на 2024 год)
 */
...
function calculateCost() {
    ...
    if (detailsDiv) {
        const paymentMethodsText = `Приём платежей: ${COMPANY_CONFIG.paymentSystems.join(', ')}`;
        const details = `
            <p style="font-size: 0.9em; color: #666;">
                <strong>Детализация расчета:</strong><br>
                Базовая стоимость: ${formatNumber(baseCost)} сом<br>
                Сложность: ×${complexityMultiplier} | Срок: ×${timelineMultiplier}<br>
                ${selectedOptions.length > 0 ? `Дополнительные опции: ${selectedOptions.join(', ')}<br>` : ''}
                <br>
                <strong>Контакты TechPro KG:</strong><br>
                📧 ${COMPANY_CONFIG.email}<br>
                📞 ${COMPANY_CONFIG.phone}<br>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log(`🚀 TechPro KG — IT-аутсорсинг в Кыргызстане`);
    console.log(`📧 Email: ${COMPANY_CONFIG.email}`);
    console.log(`📞 Телефон: ${COMPANY_CONFIG.phone}`);
    console.log(`📍 Адрес: ${COMPANY_CONFIG.address}`);
    
    if (!FORMSPREE_FORM_ID) {
        console.info('ℹ️ Demo mode: Заявки сохраняются в localStorage. Для реальной отправки на почту info@techpro.kg используйте Formspree (https://formspree.io)');
    }
});
